    ...
    public static void discover() throws IOException {
        ...
        classes.forEach(c -> {
            ...
            for (int i = 0; i < fields.length; i++) {
                ...
                if (field.isAnnotationPresent(GrpcQuote.class)) {
                    Type genericType = field.getGenericType();
                    if (!proxyMap.containsKey(genericType.getTypeName())) {
                        GrpcConsumer grpcConsumer = new GrpcConsumer();
                        Object object = grpcConsumer.proxy(genericType.getClass());
